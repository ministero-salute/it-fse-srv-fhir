...
public interface IXslTransformCTL extends Serializable {
        ...
        ResponseEntity<XslTransformUploadResponseDTO> addXslTransform(
                        ...
                        @RequestPart @Parameter(description = "Xslt version", schema = @Schema(minLength = 1, maxLength = 100)) @Size(min = 1, max = 100) @NotBlank(message = "Template Id cannot be blank") @Pattern(regexp = "[0-9]\\.[0-9]") String version,
        ...
        ResponseEntity<XslTransformResponseDTO> updateXslTransform(
                        @RequestPart @Parameter(description = "Template Id Root of the xslt", schema = @Schema(minLength = 1, maxLength = 100)) @Size(min = 1, max = 100) @NotBlank(message = "Template Id cannot be blank") String templateIdRoot,
                        @RequestPart @Parameter(description = "Xslt version", schema = @Schema(minLength = 1, maxLength = 100)) @Size(min = 1, max = 100) @NotBlank(message = "Version cannot be blank") @Pattern(regexp = "[0-9]\\.[0-9]") String version,
                        @RequestPart("file") MultipartFile file, HttpServletRequest request)
